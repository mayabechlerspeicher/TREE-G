...
from gta_node.data_formatter import DataFormatter
from gta_node.graph_data_node_level import SparseGraphData
import wandb
import threading
...
def metrics(preds, y):
    acc = sklearn.metrics.accuracy_score(np.array(preds), np.array(y))
    return acc
...
def train_valid_test_multiclass_paralel(graph, X_train, y_train, X_valid, X_test, max_attention_depth, max_graph_depth,
    ...
    gbgta = boosting_model(
        init_estimator=GTANode(graph=graph,
        ...
        base_estimator=GTANode(graph=graph,
                               max_attention_depth=max_attention_depth,
                               walk_lens=list(range(0, max_graph_depth + 1)),
                               attention_types=attention_types,
                               attention_type_sample_probability=attention_type_sample_probability
...
def run_paralel_multiclass_train_val_test(dataset, with_constant_one_feature=True,
    ...
    graph, X_train, X_valid, X_test, y_train, y_valid, y_test = dataset(formatter)
